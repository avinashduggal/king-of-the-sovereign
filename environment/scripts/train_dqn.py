...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Path to a replay_buffer.pkl saved from a previous DQN run.",
    )
    ...
    model = load_or_init_model(
        DQN, args.load_checkpoint, train_env,
        policy="MlpPolicy",
        verbose=0,
        seed=args.seed,
    ...
    log.info("DQN | preset=%s | device=cpu | from_ts=%d", args.preset, model.num_timesteps)
